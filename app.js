...
(function () {
  ...
  const statsMonthTitle = qs("#statsMonthTitle");
  const statsMonthTotal = qs("#statsMonthTotal");
  const statsMonthPrev = qs("#statsMonthPrev");
  const statsMonthNext = qs("#statsMonthNext");
  const statsMonthToday = qs("#statsMonthToday");

  let statsCursor = new Date(); // which month the category card is showing
  ...
  function renderStats() {
    ...
    // category breakdown — for whichever month statsCursor points to
    const csY = statsCursor.getFullYear();
    const csM = statsCursor.getMonth();
    const cursorMonthKey = `${csY}-${String(csM + 1).padStart(2, "0")}`;
    const monthExpenses = expenses.filter((e) => (e.date || "").startsWith(cursorMonthKey));

    const today = new Date();
    const isCurrentMonth = csY === today.getFullYear() && csM === today.getMonth();
    statsMonthTitle.textContent = isCurrentMonth
      ? "Xarajatlar toifa bo'yicha (shu oy)"
      : `Xarajatlar toifa bo'yicha — ${AY_NOMLARI[csM]} ${csY}`;
    ...
    statsMonthTotal.textContent = formatMoney(monthTotal);
